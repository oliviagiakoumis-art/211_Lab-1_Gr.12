...
class NXTColorSensor(Sensor):
    ...
    def __init__(self, port, mode="component", bp=None):
        super(NXTColorSensor, self).__init__(port, bp)
        self.set_mode(mode)
    ...
    def set_mode(self, mode: str):
        ...
        try:
            ...
            if mode.lower() == self.Mode.COMPONENT:
                self.brick.set_sensor_type(
                    self.port, BrickPi3.SENSOR_TYPE.NXT_COLOR_FULL)
            else:
                return False
            self.mode = mode.lower()
            return True
    ...
    def get_rgb(self) -> list[float]:
        ...
        if self.mode != self.Mode.COMPONENT:
            ...
            self.wait_ready()
        val = self.get_value()
        return val[1:-1] if val is not None else [None, None, None]
